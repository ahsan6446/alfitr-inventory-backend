...
function seedDb() {
  ...
  return {
    ...
    users: [
      { id: superAdminId, name: 'Super Admin', username: 'admin', passwordHash: defaultPasswordHash, role: 'Super Admin', active: true, mustChangePassword: true, createdAt: now },
    ],
    items,
    movements: [],
    clients: [
...
function migrate(state) {
  ...
  else if (state.meta.schemaVersion < 2) { state.meta.schemaVersion = 2; changed = true; }
