...
function seedDb() {
  ...
  return {
    ...
    pushSubscriptions: [],
    pushConfig: { publicKey: vapidKeys.publicKey, privateKey: vapidKeys.privateKey },
  };
}
...
function migrate(state) {
  ...
  ensure(state, 'materialRequests', []);
  ensure(state, 'materialRequestCounter', 0);
  ensure(state, 'vendors', []);
  ensure(state, 'purchaseRequests', []);
  ensure(state, 'purchaseRequestCounter', 0);
  ensure(state, 'purchaseOrders', []);
  ensure(state, 'purchaseOrderCounter', 0);
  ensure(state, 'delayReports', []);
  ensure(state, 'delayReportCounters', {});
  ...
  ensure(state, 'pushSubscriptions', []);
  if (!state.pushConfig || !state.pushConfig.publicKey) {
    // VAPID keys authenticate this server to push services (Chrome/Firefox/etc).
    // Generated once per install and reused forever after — regenerating them would
    // silently invalidate every existing subscription.
    const webpush = require('web-push');
    const keys = webpush.generateVAPIDKeys();
    state.pushConfig = { publicKey: keys.publicKey, privateKey: keys.privateKey };
    changed = true;
  }
  ...
  if (state.roles) {
    for (const role of Object.keys(state.roles)) {
      ...
      if (!('manageMaterialRequests' in state.roles[role])) {
        state.roles[role].manageMaterialRequests = ['Super Admin', 'Admin', 'Storekeeper', 'Engineer'].includes(role);
        changed = true;
      }
      if (!('manageProcurement' in state.roles[role])) {
        state.roles[role].manageProcurement = (role === 'Super Admin' || role === 'Admin');
        changed = true;
      }
      if (!('manageReports' in state.roles[role])) {
        state.roles[role].manageReports = ['Super Admin', 'Admin', 'Engineer'].includes(role);
        changed = true;
      }
      if (!('manageClients' in state.roles[role])) {
        state.roles[role].manageClients = ['Super Admin', 'Admin'].includes(role);
        changed = true;
      }
      if (!('manageFM' in state.roles[role])) {
        state.roles[role].manageFM = ['Super Admin', 'Admin', 'Engineer'].includes(role);
        changed = true;
      }
    }
    if (!state.roles['Supervisor']) {
      state.roles['Supervisor'] = { viewPricing:false, editPricing:false, exportPricing:false, viewStockValue:false, viewProfitMargin:false, manageStock:false, createDN:false, manageInventory:false, manageUsers:false, allowNegativeStock:false, manageQuotations:false, manageMaterialRequests:true, manageProcurement:false, manageReports:true, manageClients:false, manageFM:true };
      changed = true;
    }
    if (!state.roles['Technician']) {
      state.roles['Technician'] = { viewPricing:false, editPricing:false, exportPricing:false, viewStockValue:false, viewProfitMargin:false, manageStock:false, createDN:false, manageInventory:false, manageUsers:false, allowNegativeStock:false, manageQuotations:false, manageMaterialRequests:false, manageProcurement:false, manageReports:false, manageClients:false, manageFM:true };
      changed = true;
    }
    if (!state.roles['Coordinator']) {
      state.roles['Coordinator'] = { viewPricing:false, editPricing:false, exportPricing:false, viewStockValue:false, viewProfitMargin:false, manageStock:false, createDN:true, manageInventory:false, manageUsers:false, allowNegativeStock:false, manageQuotations:false, manageMaterialRequests:true, manageProcurement:false, manageReports:true, manageClients:true, manageFM:true };
      changed = true;
    }
  }
  if (!state.meta) { state.meta = { schemaVersion: 9 }; changed = true; }
  else if (state.meta.schemaVersion < 9) { state.meta.schemaVersion = 9; changed = true; }
