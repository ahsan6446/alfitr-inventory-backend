...
function migrate(state) {
  ...
  if (state.roles) {
    for (const role of Object.keys(state.roles)) {
      ...
      // New permissions for FM and Clients
      if (!('manageClients' in state.roles[role])) {
        state.roles[role].manageClients = ['Super Admin', 'Admin', 'Coordinator'].includes(role);
        changed = true;
      }
      if (!('manageFM' in state.roles[role])) {
        state.roles[role].manageFM = ['Super Admin', 'Admin', 'Engineer', 'Supervisor', 'Technician', 'Coordinator'].includes(role);
        changed = true;
      }
    }
    // Add new roles if missing
    const newRoles = {
    ...
    for (const [role, perms] of Object.entries(newRoles)) {
      if (!state.roles[role]) {
