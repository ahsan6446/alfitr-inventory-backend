...
function drawHeader(doc, co, q) {
  ...
  doc.fontSize(13).fillColor(TEAL).font('Helvetica-Bold').text(co.name || '', textX, MARGIN, { width: nameWidth });
  const afterNameY = doc.y + 2;
  const contactLine = [co.address, co.phone, co.email].filter(Boolean).join(' · ');
  doc.fontSize(8).fillColor(GRAY).font('Helvetica').text(contactLine, textX, afterNameY, { width: nameWidth });
  ...
  doc.fontSize(8).fillColor(GRAY).font('Helvetica')
    .text((q.status === 'PendingApproval' ? 'PENDING APPROVAL' : q.status || '').toUpperCase(), pageWidth - MARGIN - titleBlockW, MARGIN + 32, { width: titleBlockW, align: 'right' });
...
function drawFooter(doc, co, pageNum, totalPages) {
  ...
  doc.fontSize(7.5).fillColor(GRAY).font('Helvetica')
    .text(co.name || '', MARGIN, y + 6, { width: 300 })
    .text(`Page ${pageNum} of ${totalPages}`, pageWidth - MARGIN - 150, y + 6, { width: 150, align: 'right' });
}
...
function drawMetaGrid(doc, co, q, cursor) {
  ...
  const col2X = MARGIN + 230;    // right column moved closer to the left column instead of splitting the page exactly in half
  const valueWidth = 220;        // capped rather than stretched, so short values don't leave a large empty trailing gap
  const rows = [
  ...
  for (const [k1, v1, k2, v2] of rows) {
    const h1 = doc.heightOfString(v1, { width: valueWidth - 10, fontSize: 9.5 });
    const h2 = doc.heightOfString(v2, { width: valueWidth - 10, fontSize: 9.5 });
    ...
    doc.fontSize(9.5).fillColor(NAVY).font('Helvetica').text(v1, MARGIN, y + 11, { width: valueWidth - 10 });
    doc.fontSize(9.5).fillColor(NAVY).font('Helvetica').text(v2, col2X, y + 11, { width: valueWidth - 10 });
    y += rowHeight;
  }
  cursor.y = y + 4;
}
