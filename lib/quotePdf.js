...
function drawMetaGrid(doc, co, q, cursor) {
  ...
  const col2X = MARGIN + 200;          // right column starts closer to the left column
  const col1Width = 200 - 10;          // left column value width
  const col2Width = pageRight - col2X - 0; // right column extends all the way to the true right margin — matches the item table's width below, so there's no leftover blank rectangle
  const rows = [
