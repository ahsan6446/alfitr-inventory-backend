...
// Header — logo left + company name beside it, doc title right
function drawHeader(doc, co, q) {
  ...
  const logoH       = 44 * logoScale;
  const titleBlockW = 150;
  let   logoW       = 0;

  // Logo — left aligned, vertically centered in header
  if (logoPath) {
    try {
      const props = doc.openImage(logoPath);
      logoW = (props.width / props.height) * logoH;
      const logoY = MARGIN - 4; // slightly above margin to keep gap from divider
      doc.image(logoPath, MARGIN, logoY, { height: logoH });
    } catch (e) { console.warn('PDF logo embed failed', e.message); }
  }

  // Company name — starts after logo with a clean gap, no text cursor artifact
  const nameX = MARGIN + logoW + 14;
  const nameW = pageWidth - nameX - MARGIN - titleBlockW - 10;
  const nameY = MARGIN + (HEADER_H - 20) / 2 - 6;
  doc.save();
  doc.fontSize(13).fillColor(TEAL).font('Helvetica-Bold');
  doc.text(co.name || '', nameX, nameY, { width: nameW, align: 'left', lineBreak: false });
  doc.restore();

  // Doc type + number + status — right block
  doc.fontSize(15).fillColor(NAVY).font('Helvetica-Bold')
    .text('QUOTATION', pageWidth - MARGIN - titleBlockW, MARGIN + 4, { width: titleBlockW, align: 'right' });
  doc.fontSize(9).fillColor('#D96F24').font('Helvetica-Bold')
    .text(q.quotationNumber || '(not yet sent)', pageWidth - MARGIN - titleBlockW, MARGIN + 22, { width: titleBlockW, align: 'right' });
  doc.fontSize(8).fillColor(GRAY).font('Helvetica')
    .text(
      (q.status === 'PendingApproval' ? 'PENDING APPROVAL' : q.status || '').toUpperCase() +
      (q.revisionOf ? ` · REV ${q.revisionNumber}` : ''),
      pageWidth - MARGIN - titleBlockW, MARGIN + 35,
      { width: titleBlockW, align: 'right' }
    );

  // Divider — at bottom of header with breathing room
  const dividerY = MARGIN + HEADER_H - 10;
...
function drawFooter(doc, co, pageNum, totalPages) {
  ...
  const textY  = y + 7;
  const pageW  = 55;   // fixed width for page number on far right
  const col3W  = 130;  // email column
  const col2W  = 100;  // phone column
  const col1W  = contentW - col2W - col3W - pageW - 12; // address gets remaining space

  // Left — office address (single line, truncated if too long)
  doc.fontSize(7).fillColor(GRAY).font('Helvetica')
    .text(co.address || '', MARGIN, textY, { width: col1W, lineBreak: false, ellipsis: true });

  // Center — phone
  doc.fontSize(7).fillColor(GRAY).font('Helvetica')
    .text(co.phone || '', MARGIN + col1W + 6, textY, { width: col2W, align: 'center', lineBreak: false });

  // Right — email
  doc.fontSize(7).fillColor(GRAY).font('Helvetica')
    .text(co.email || '', MARGIN + col1W + col2W + 12, textY, { width: col3W, align: 'right', lineBreak: false });

  // Far right — page number
  doc.fontSize(7).fillColor(GRAY).font('Helvetica')
    .text(`Page ${pageNum} of ${totalPages}`, pageWidth - MARGIN - pageW, textY, { width: pageW, align: 'right', lineBreak: false });
}
