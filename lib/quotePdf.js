...
// Draws the letterhead — logo left, company name center, doc title right
function drawHeader(doc, co, q) {
  ...
  const logoH     = 40 * logoScale;
  const logoW_est = 60 * logoScale;
  ...
  const centerX = MARGIN + logoW_est;
  const centerW = pageWidth - MARGIN * 2 - logoW_est - titleBlockW;
  doc.fontSize(14).fillColor(TEAL).font('Helvetica-Bold')
    .text(co.name || '', centerX, MARGIN + 12, { width: centerW, align: 'center' });

  // Doc type + number + status — right
...
function drawFooter(doc, co, pageNum, totalPages) {
  ...
  doc.fontSize(7).fillColor(GRAY).font('Helvetica')
    .text(co.address || '', MARGIN, textY, { width: colW, align: 'left', lineBreak: false });

  // Center — phone
  doc.fontSize(7).fillColor(GRAY).font('Helvetica')
    .text(co.phone || '', MARGIN + colW, textY, { width: colW, align: 'center', lineBreak: false });

  // Right — email + page number
  doc.fontSize(7).fillColor(GRAY).font('Helvetica')
    .text(co.email || '', MARGIN + colW * 2, textY, { width: colW - 40, align: 'right', lineBreak: false });

  doc.fontSize(7).fillColor(GRAY).font('Helvetica')
    .text(`Page ${pageNum} of ${totalPages}`, pageWidth - MARGIN - 35, textY, { width: 35, align: 'right', lineBreak: false });
}
