...
const MARGIN   = 40;
const HEADER_H = 80;   // enough room for logo + gap above divider
const FOOTER_H = 40;
...
// ── Header ────────────────────────────────────────────────────────────
// Logo left, company name beside logo, QUOTATION block right
function drawHeader(doc, co, q) {
  const pageWidth   = doc.page.width;
  const logoPath    = logoAbsPath(co);
  const logoScale   = { small: 0.8, medium: 1, large: 1.3 }[co.logoSize] || 1;
  const dividerY    = MARGIN + HEADER_H - 14;   // divider position
  const maxLogoH    = dividerY - MARGIN - 6;     // logo must not go below divider minus 6px gap
  const logoH       = Math.min(50 * logoScale, maxLogoH);
  const titleBlockW = 150;
  let   logoW       = 0;

  // Logo — top left, strictly above divider
  if (logoPath) {
    try {
      const props = doc.openImage(logoPath);
      logoW = (props.width / props.height) * logoH;
      doc.image(logoPath, MARGIN, MARGIN, { height: logoH });
    } catch (e) { console.warn('PDF logo embed failed', e.message); }
  }

  // Company name — right of logo, vertically centered in available space
  const nameX = MARGIN + logoW + 12;
  const nameW = pageWidth - nameX - MARGIN - titleBlockW - 10;
  const nameY = MARGIN + logoH / 2 - 7;
  doc.fontSize(13).fillColor(TEAL).font('Helvetica-Bold')
     .text(co.name || '', nameX, nameY, { width: nameW, lineBreak: false });

  // QUOTATION block — right
  doc.fontSize(15).fillColor(NAVY).font('Helvetica-Bold')
     .text('QUOTATION', pageWidth - MARGIN - titleBlockW, MARGIN, { width: titleBlockW, align: 'right' });
  doc.fontSize(9).fillColor('#D96F24').font('Helvetica-Bold')
     .text(q.quotationNumber || '(not yet sent)', pageWidth - MARGIN - titleBlockW, MARGIN + 20, { width: titleBlockW, align: 'right' });
  doc.fontSize(8).fillColor(GRAY).font('Helvetica')
     .text(
       (q.status === 'PendingApproval' ? 'PENDING APPROVAL' : q.status || '').toUpperCase() +
       (q.revisionOf ? ` · REV ${q.revisionNumber}` : ''),
       pageWidth - MARGIN - titleBlockW, MARGIN + 33,
...
// ── Footer ────────────────────────────────────────────────────────────
// Left: address | Center: phone | Right: email  +  page number far right
function drawFooter(doc, co, pageNum, totalPages) {
  ...
  const col3W   = 120;
  const col2W   = 100;
  const col1W   = contentW - col2W - col3W - pageW - 15;

  // Address — left, no wrap
  doc.fontSize(7).fillColor(GRAY).font('Helvetica')
     .text(co.address || '', MARGIN, textY, { width: col1W, lineBreak: false, ellipsis: true });

  // Phone — center
  doc.fontSize(7).fillColor(GRAY).font('Helvetica')
     .text(co.phone || '', MARGIN + col1W + 5, textY, { width: col2W, align: 'center', lineBreak: false });

  // Email — right of center
  doc.fontSize(7).fillColor(GRAY).font('Helvetica')
     .text(co.email || '', MARGIN + col1W + col2W + 10, textY, { width: col3W, align: 'right', lineBreak: false });

  // Page number — far right
  doc.fontSize(7).fillColor(GRAY).font('Helvetica')
     .text(pageStr, pageWidth - MARGIN - pageW, textY, { width: pageW, align: 'right', lineBreak: false });
}
...
function drawMetaGrid(doc, co, q, cursor) {
  ...
  const col2Width = (doc.page.width - MARGIN) - col2X;
...
function itemTableCols(pageWidth) {
  ...
  return [
    ...
    { key: 'unitPrice',   label: 'Unit Price',  width: w * 0.16, align: 'right' },
...
function drawLineItems(doc, co, q, cursor, lines, groupLabel) {
  ...
  if (groupLabel) {
    ensureSpace(doc, co, q, cursor, 28);
    doc.fontSize(9.5).fillColor(NAVY).font('Helvetica-Bold').text(groupLabel, MARGIN, cursor.y);
    cursor.y = doc.y + 4;
  }
  ensureSpace(doc, co, q, cursor, 18);
  cursor.y = drawTableHeader(doc, cols, cursor.y);
  lines.forEach((l, idx) => {
    const rowText = l.description || '';
    const descW   = cols[0].width - 8;
    const rowH    = Math.max(16, doc.heightOfString(rowText, { width: descW, fontSize: 8 }) + 8);
    ensureSpace(doc, co, q, cursor, rowH);
    if (cursor.y === contentTop()) cursor.y = drawTableHeader(doc, cols, cursor.y);
    const shaded = idx % 2 === 1;
    const totalW = cols.reduce((s, c) => s + c.width, 0);
    if (shaded) doc.rect(MARGIN, cursor.y, totalW, rowH).fill(LIGHT);
    let x = MARGIN;
    doc.fontSize(8).fillColor(NAVY).font('Helvetica');
    for (const c of cols) {
      let val = '';
      if (c.key === 'description') val = l.description || '';
      else if (c.key === 'brand')  val = l.brand || '';
      else if (c.key === 'unit')   val = l.unit || '';
      else if (c.key === 'qty')    val = String(l.qty ?? '');
      else if (c.key === 'unitPrice') val = fmtMoney(l.unitPrice);
      else if (c.key === 'total')     val = fmtMoney(lineTotal(l));
      doc.text(val, x + 4, cursor.y + 4, { width: c.width - 8, align: c.align || 'left', lineBreak: true });
      x += c.width;
    }
    cursor.y += rowH;
  });
...
function drawTotals(doc, co, q, cursor, totals) {
  ensureSpace(doc, co, q, cursor, 80);
  const boxW = 200;
  ...
  const line = (label, value, bold) => {
    ...
    doc.text(label, x, y, { width: boxW * 0.55 });
    doc.text(value, x + boxW * 0.55, y, { width: boxW * 0.45, align: 'right' });
    y += 15;
  };
